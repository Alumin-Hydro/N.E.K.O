...
3. 填写 HAPI 地址和 token。token 输入框始终为空；保存后的界面只显示“已配置”和末四位。
...
默认只允许回环地址。使用远程 HAPI 时，必须在面板显式开启远程端点选项，并应使用 HTTPS、受限网络和最小权限凭据。
...
| `POST /api/sessions/{sessionId}/resume` | 恢复非活动会话 | 保留详情中已经验证为安全的 `permissionMode`；可读取新的 `sessionId`，恢复后会重新校验会话策略 |
| `POST /api/sessions/{sessionId}/reopen` | 恢复的版本兼容后备 | 仅在 `resume` 返回 404 时尝试；保留安全权限模式并重新校验返回的会话 |
...
```text
测试连接 → 刷新会话 → 选择在线机器、provider 和允许目录
...
- **安全的网络默认值。** 默认地址是回环 HTTP 地址。非 `localhost`、`127.0.0.0/8` 或 `::1` 端点必须显式开启远程访问；URL 禁止用户信息、查询参数和片段。
...
- **凭据不回显。** token 单独存储，面板和工具只看到配置状态及末四位；空白保存保留旧 token，清除需要显式操作。连接器代码不记录 token，并会从自身错误、UI、持久化元数据和模型结果中剔除配置凭据。token 经 N.E.K.O 的 `/runs` 管理通道写入，部署方仍应避免在宿主启用会记录原始入口参数的调试日志。
